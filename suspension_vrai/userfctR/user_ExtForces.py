...
def user_ExtForces(PxF, RxF, VxF, OMxF, AxF, OMPxF, mbs_data, tsim, ixF):
    ...
    """Compute an user-specified external force with angular influence, considering wheel radius."""
    Fx = 0.0
    Fy = 0.0
    Fz = 0.0
    
    if ixF == mbs_data.extforce_id['ExtForce_0']:
        K  = mbs_data.user_model['Pneu']['K']
        D  = mbs_data.user_model['Pneu']['D']
        R  = mbs_data.user_model['Pneu']['R0']
    ...
    # Mise à jour du tableau de sortie
